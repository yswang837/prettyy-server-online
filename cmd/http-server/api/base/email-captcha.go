...
func (s *Server) GetIdentifyCodeByEmail(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusBadRequest, ginConsulRegister.Response{Code: 4000040, Message: "邮件发送失败，请稍后重试或联系客服"})
		return
	}
	if err = user.SetExByEmail(email, iCode); err != nil {
		ctx.JSON(http.StatusBadRequest, ginConsulRegister.Response{Code: 4000041, Message: "设置邮箱验证码失败"})
		return
	}
