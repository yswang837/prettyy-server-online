...
func (s *Server) ColumnList(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusBadRequest, ginConsulRegister.Response{Code: 4000361, Message: "获取反向索引失败"})
		return
	}
	...
	for _, cidInverted := range cidInvertedList {
		...
		if err != nil {
			ctx.JSON(http.StatusBadRequest, ginConsulRegister.Response{Code: 4000362, Message: "获取专栏数据失败"})
			return
		}
		result[col.Cid] = col.Title
	}
	if len(result) == 0 {
		ctx.JSON(http.StatusBadRequest, ginConsulRegister.Response{Code: 4000363, Message: "未查询到专栏数据"})
		return
	}
